...
def fetch_measurements_for_day(parent_post_hash):
    ...
    payload = {"PostHashHex": parent_post_hash, "CommentOffset": 0, "CommentLimit": 100}
    try:
        resp = requests.post(url, json=payload)
        resp.raise_for_status()
        post_data = resp.json()
...
if __name__ == "__main__":
    ...
    node_measurements = {node: [] for node in NODES}
    for comment in comments:
        body = comment.get("Body", "")
        node = comment.get("PostExtraData", {}).get("Node")
        elapsed = None
        timestamp = None
        # Try to parse elapsed and timestamp from body
        for line in body.splitlines():
            if line.startswith("Elapsed:"):
                try:
                    elapsed = float(line.split(":",1)[1].split()[0])
                except:
                    pass
            if line.startswith("Timestamp:"):
                try:
                    ...
                    pass
        if node and elapsed is not None and timestamp:
            node_measurements[node].append((timestamp, elapsed))

    # Plot graph for the selected date + GRAPH_DAYS
    plt.figure(figsize=(8, 4))
    cutoff = post_dt - datetime.timedelta(days=0)  # start from selected date
    end_cutoff = post_dt + datetime.timedelta(days=GRAPH_DAYS)
    for node in NODES:
        filtered = [(t, e) for t, e in node_measurements[node]
        ...
        print(f"Graph data for {node}: {len(elapsed)} measurements from {date_str} + {GRAPH_DAYS} days")
    plt.xlabel("Time")
    plt.ylabel("Elapsed (sec)")
    plt.title(f"Node Performance - {date_str} + {GRAPH_DAYS} Days")
    plt.legend()
    plt.tight_layout()
